...
`configure` may record `unknown_node`, `unknown_parameter`, `unavailable`, or `value_available` and still succeed. While active, callbacks fire for remote parameter changes. While inactive, the snapshot still updates but user callbacks stay gated.
